...
function Projects({ userName }) {
  ...
  return (
    <div className="page-container">
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '8rem' }}>
        {flagshipProjects.map((project, i) => (
          <div key={i} style={{ 
            ...
            <div className="glass-card" style={{ padding: '0', overflow: 'hidden', height: '500px', border: '1px solid var(--tan-glow)' }}>
              <img 
                src={project.image} 
                alt={project.title} 
            ...
            <div style={{ padding: '1rem' }}>
              ...
              <h2 style={{ fontSize: '3.5rem', margin: '0.5rem 0', fontWeight: '800' }}>{project.title}</h2>
              <p style={{ color: 'var(--muted)', marginBottom: '2rem', fontSize: '0.9rem', letterSpacing: '0.1em' }}>
                {project.location.toUpperCase()} | {project.status.toUpperCase()}
              </p>
              
              <p className="subtitle" style={{ maxWidth: '100%', marginBottom: '2rem' }}>
                {project.description}
              </p>
              ...
              <div style={{ 
                ...
                <div>
                  <h4 style={{ color: 'var(--walnut)', marginBottom: '1rem', textTransform: 'uppercase', fontSize: '0.8rem', letterSpacing: '0.1em' }}>Key Features</h4>
                  <ul style={{ listStyle: 'none', padding: '0', margin: '0' }}>
                    {project.features.map((feat, idx) => (
                      <li key={idx} style={{ fontSize: '0.9rem', marginBottom: '0.5rem', color: 'var(--ink)', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                ...
                <div>
                  <h4 style={{ color: 'var(--walnut)', marginBottom: '1rem', textTransform: 'uppercase', fontSize: '0.8rem', letterSpacing: '0.1em' }}>Investment</h4>
                  <div style={{ fontSize: '1.8rem', fontWeight: '800', color: 'var(--tan)' }}>{project.investment.price}</div>
                  <div style={{ fontSize: '0.85rem', color: 'var(--muted)', marginTop: '0.3rem' }}>{project.investment.plan}</div>
                </div>
              </div>

              <button className="glass-btn primary-btn" style={{ padding: '1rem 2.5rem', fontSize: '0.9rem' }}>
      ...
      <div className="glass-card" style={{ marginTop: '10rem', textAlign: 'center', padding: '4rem' }}>
        <h3 style={{ fontSize: '2rem', marginBottom: '1rem' }}>Have a specialized project in mind?</h3>
        <p className="subtitle" style={{ margin: '0 auto' }}>
